...
public class LeffaApplication {
	...
	public CommandLineRunner leffaDemo(LeffaRepository leffaRepository, StudioRepository studioRepository, GenreRepository genreRepository, UserRepository urepository) {
		return (args) -> {
			...
			leffaRepository.save(new Leffa("Lord of the rings", 2001, "Peter Jackson", studioRepository.findByName("Wingnut").get(0), genreRepository.findByName("Fantasy").get(0)));
			leffaRepository.save(new Leffa("Star Wars", 1981, "George Lucas", studioRepository.findByName("Lucas").get(0), genreRepository.findByName("Sci-Fi").get(0)));
